 ...
 #                           with open('file.json', 'r', encoding='utf_8') as f:
  #                              H = json.loads(f.read())
            ...
            for d in range(testing_days):
                ...
                print(len(target_station))
                out_folder = orig_folder + dest_folder + '/' + target_station +'_day' + str(d + 1)
                #date = test_days[d][0][len(target_station) + 2:len(target_station) + 10]
               # print(date)
                ...
                predict_mse = mean_squared_error(y_day, y_pred_day)
                print(predict_mse)
                acc_mse=acc_mse+mean_squared_error(y_day, y_pred_day)
                predict_mae = mean_absolute_error(y_day, y_pred_day)
                persistence_mse = mean_squared_error(y_day, y_pers)
                acc_mse_persistence=acc_mse_persistence+mean_squared_error(y_day, y_pers)
                persistence_mae = mean_absolute_error(y_day, y_pers)

                predict_mse_list.append(predict_mse)
                predict_mae_list.append(predict_mae)
                persistence_mse_list.append(persistence_mse)
                persistence_mae_list.append(persistence_mae)
                ...
                plt.title('Predicted and real relative values for day ' + str(d) + '\nForecasting prediction: ')
                plt.xticks(xs, hours, rotation='vertical')

                plt.savefig(out_folder +output_text + '_day_' + str(d) + '.png', bbox_inches='tight', dpi=300)
                plt.close()

                plt.plot(y_pred_day, label='Real')
                plt.plot(y_pers, label='persistence')
                plt.legend(loc='best', fancybox=True, framealpha=0.5)
                plt.title('Persistence and real relative values for day ' + str(d) + '\nForecasting persistence: ')
                plt.xticks(xs, hours, rotation='vertical')
                plt.savefig(out_folder + output_text + '_day_' + str(d) + 'persistence' + '.png', bbox_inches='tight', dpi=300)
                plt.close()
                ...
                y_pers_df = pd.DataFrame(y_pers)

                y_df = pd.concat([y_day_df, y_pred_day_df, y_pers_df], axis=1)
                y_df.columns = ['real', 'predicted', 'persistence']
                y_df.to_csv(out_folder + output_text + '_day' + str(d) + '.csv', header=True, index=False)
                print('Day finished!\n')
            print('Creating csv...\n')
            MSE_Final=acc_mse/(testing_days+1)
            RMSE=math.sqrt( (MSE_Final))
            print(RMSE)
            MSE_Final_persistence=acc_mse_persistence/(testing_days+1)
