...
x_all = pd.read_csv(orig_folder+'/../../../X_tr_val.csv')
###y_original = pd.read_csv(orig_folder+'/Y_tr_val.csv')
...
#3#x_train = x_all.iloc[days_train]
x_train = x_all
#3#x_val_original = x_all.iloc[days_validation]
x_val_original = x_all
y_val_original = y_original
y_train = y_original
...
#    x_train = pd.concat([x_train,x_original.iloc[day:(day+1)]],ignore_index=True)
#    y_train = pd.concat([y_train,y_original.iloc[day*day_length:(day+1)*day_length]],ignore_index=True)
#    y_train = pd.concat([y_train,y_original.iloc[day:(day+1)]],ignore_index=True)
...
#    x_val_original = pd.concat([x_val_original,x_original.iloc[day:(day+1)]],ignore_index=True)
#    y_val_original = pd.concat([y_val_original,y_original.iloc[day*day_length:(day+1)*day_length]],ignore_index=True)
#    y_val_original = pd.concat([y_val_original,y_original.iloc[day:(day+1)]],ignore_index=True)

## drop indexes here?
#x_train = x_train.drop('index',axis=1)
#y_train = y_train.drop('index',axis=1)
#x_val_original = x_val_original.drop('index',axis=1)
#y_val_original = y_val_original.drop('index',axis=1)
...
#TRAIN SIZE:
    ...
    for ts in train_size:
        ...
        y_t_index = x_train.sample(n_rows,random_state=seed).index.values
        y_t_index = days_train
        print('y_t_index length {}\n'.format(len(y_t_index)))
        y_t_index_valid = y_t_index[(y_t_index % day_length) < (day_length - hor_pred_indices)]
        print('y_t_index_valid length {}\n'.format(len(y_t_index_valid)))
        ...
        print('y_t end process len {} and data {}'.format(len(y_t),y_t))
        ...
        print('y_valid_index {}'.format(y_val_index))
        y_val_index = x_val_original.index.values
        y_val_index = days_validation
        print('y_valid_index {}'.format(y_val_index))
        y_val_index_valid = y_val_index[(y_val_index % day_length) < (day_length - hor_pred_indices)]
        print('y_val_index len {} and data {}'.format(len(y_val_index_valid), y_val_index_valid))
        y_pred_persistence_val = np.ravel(y_val_original.iloc[y_val_index_valid])
        print('ypred_persistence_val len {} and data {}'.format(len(y_pred_persistence_val), y_pred_persistence_val))
        print('Building X matrix...Same thing as before...\n')
        print('x_train index {}'.format(x_train.index.values))
        ...
        print('x_t len {} and data {}'.format(len(x_t), x_t))
        print('x_val len {} and data {}'.format(len(x_val), x_val))
        print('y_val len {} and data {}'.format(len(y_val), y_val))
#STATIONS TO SELECT:
        ...
        for ft in feature_values:
            ...
            for av in alpha_values:
                ...
                if(os.path.exists(model_filename)):
                    ...
                    if(os.path.exists('file22.json')):
                        with open('file22.json', 'r', encoding='utf_8') as f:
                            H = json.loads(f.read())
